...
def updateBookRoute():
    if request.method == "POST":
        bookID = str(request.form.get("bookID"))
        price1 = str(request.form.get("price1"))  # Current price
        price2 = str(request.form.get("price2"))  # New price
        quantity = str(request.form.get("quantity"))
        response = updateBook(mysql, bookID, price1, price2,quantity)
        booksData = allBooks(mysql)
        genreData = allGenre(mysql)
        return render_template("books.html", booksData=booksData, genreData=genreData, response=response)
    return redirect(url_for("booksRoute"))
...
def buyBookRoute(bookID):
    if request.method == "POST":
        ...
        # Fetch the book's details (including stock) from the database
        cur = mysql.connection.cursor()
        cur.execute("SELECT totalStock FROM inventory WHERE bookID = %s", (bookID,))
        ...
        if quantity > availableStock:
            ...
            error_message = f"Sorry, only {availableStock} copies are available in stock."
            return render_template("bookdetail.html", bookData=bookData, error_message=error_message)
        
        # Proceed with the price calculation if stock is sufficient
        
        totalPrice = int(bookData[1]) * quantity
...
def contactUsRoute():
    if request.method == "POST":
        ...
        message = str(request.form.get("subject"))
